...
local function IsTimerRunning(ply)
	return not (ply.Practice or ply.SSJ_InZone)
end
...
-- Bash Menu
local SourceModMenu = include("sv_menu.lua")
local BashStats = {}

local function ShowBashStats(client, target)
	if not IsValid(target) then
		PrintToChat(client, "Selected player no longer ingame.")
		return
	end

	g_target[client] = target

	local menu = SourceModMenu(BashStats.MainMenu)
	menu:SetTitle("[BASH] Select stats for " .. target:Nick())
	menu:AddItem("start", "Start Strafe (Original)")
	menu:AddItem("end", "End Strafe")
	menu:AddItem("keys", "Key Switch")
	menu:AddItem("scroll", "Scroll stats")
	menu:AddItem("gain", string.format("Current gains: %.2f", GetGainPercent(GetBashData(target))))
	menu:AddItem("cvars", "ConVars stats")
	menu:AddItem("motd", "Motd test")
	menu:AddItem("cancel")
	menu:Display(client, 0)
end

function BashStats.MainMenu(menu, ply, action, key)
	if action == "select" then
		if key == "start" then
			BashStats.StartStrafes(ply)
		elseif key == "end" then
			BashStats.EndStrafes(ply)
		elseif key == "keys" then
			BashStats.KeySwitches(ply)
		elseif key == "scroll" then
			BashStats.ScrollStats(ply, false)
		elseif key == "gain" then
			ShowBashStats(ply, g_target[ply])
		elseif key == "motd" then
			PerformMOTDTest(ply)
			ShowBashStats(ply, g_target[ply])
		elseif key == "cvars" then
			BashStats.ConVars_Menu(ply)
		end
	end
...
function BashStats.ConVars_Menu(client)
	local target = g_target[client]
	if not IsValid(target) then
		return PrintToChat(client, "Selected player no longer ingame.")
	end

	local menu = SourceModMenu(BashStats.ConVarsMenu)
	menu:SetTitle("[BASH] ConVars menu: " .. target:Nick())
	menu:AddItem("values", "See values")
	menu:AddItem("checks", "Checks count")
	menu:AddItem("changes", "Changes count")
	menu:AddItem("reset", "Reset convar stats")
	menu:AddItem("cancel")
	menu:Display(client, 0)
end

function BashStats.ConVarsMenu(menu, ply, action, key)
	if action == "select" then
		if key == "values" then
			BashStats.ConVars_Values(ply)
		elseif key == "checks" then
			BashStats.ConVars_Checks(ply)
		elseif key == "changes" then
			BashStats.ConVars_Changes(ply)
		elseif key == "reset" then
			BashStats.ConVars_Menu(ply)
			InitConvars(g_target[ply], 0)
		end
	end
...
function BashStats.ConVarsMenuBack(menu, ply, action, key)
	if action == "cancel" then
		BashStats.ConVars_Menu(ply)
	end
	if action == "end" then menu:Close() end
...
function BashStats.ConVars_Values(client)
	local target = g_target[client]
	if not IsValid(target) then
		return PrintToChat(client, "Selected player no longer ingame.")
	end

	local vals = GetBashData(target).cvar
	local menu = SourceModMenu(BashStats.ConVarsMenuBack)
	menu:SetTitle("[BASH] Convars values: " .. target:Nick())
	for idx, cv in ipairs(g_convarIds) do
		menu:AddItem("", cv .. " = " .. tostring(vals[cv] or "nil"), true)
	end
	menu:AddItem("cancel")
	menu:Display(client, 0)
end

function BashStats.ConVars_Checks(client)
	local target = g_target[client]
	if not IsValid(target) then
		return PrintToChat(client, "Selected player no longer ingame.")
	end

	local cnt = GetBashData(target).cvarCheckedCount
	local menu = SourceModMenu(BashStats.ConVarsMenuBack)
	menu:SetTitle("[BASH] Convars checks: " .. target:Nick())
	for idx, cv in ipairs(g_convarIds) do
		menu:AddItem("", cv .. " = " .. (cnt[cv] or 0), true)
	end
	menu:AddItem("cancel")
	menu:Display(client, 0)
end

function BashStats.ConVars_Changes(client)
	local target = g_target[client]
	if not IsValid(target) then
		return PrintToChat(client, "Selected player no longer ingame.")
	end

	local cnt = GetBashData(target).cvarChangedCount
	local menu = SourceModMenu(BashStats.ConVarsMenuBack)
	menu:SetTitle("[BASH] Convars changes: " .. target:Nick())
	for idx, cv in ipairs(g_convarIds) do
		menu:AddItem("", cv .. " = " .. (cnt[cv] or 0), true)
	end
	menu:AddItem("cancel")
...
function BashStats.BackMenuHandler(menu, ply, action, key)
	if action == "cancel" then
		ShowBashStats(ply, g_target[ply])
	end
	if action == "end" then menu:Close() end
...
function BashStats.StartStrafes(client)
    local target = g_target[client]
    if not IsValid(target) then
        return PrintToChat(client, "Selected player no longer ingame.")
    end
    ...
    for idx = 0, MAX_FRAMES - 1 do
        if data.bStartStrafe_IsRecorded[idx] == true then
            table.insert(array, data.iStartStrafe_Stats[idx].Difference)
            local j = data.iStartStrafe_Stats[idx].Button
    ...
    if size == 0 then 
        PrintToChat(client, string.format("Player '%s' has no start strafe stats.", target:Nick())) 
        return 
    end

    local startStrafeMean, startStrafeSD = 0, 0
    if size > 0 then
        startStrafeMean = GetAverage(array, size)
        startStrafeSD = StandardDeviation(array, size, startStrafeMean)
    end

    local menu = SourceModMenu(BashStats.BackMenuHandler)
    menu:SetTitle(string.format("[BASH] Start Strafe stats: %s\nAverage: %.2f | Deviation: %.2f\nA: %d, D: %d, W: %d, S: %d",
        target:Nick(), startStrafeMean, startStrafeSD, buttons[2], buttons[3], buttons[0], buttons[1]))
...
local function UpdateGains(data, vel, angles, buttons)
	...
	if client:OnGround() then
		...
		data.iTicksOnGround = data.iTicksOnGround + 1
	else
		if data.iTicksOnGround ~= 0 then
			OnPlayerJump(client, data)
		end

		if client:GetMoveType() == MOVETYPE_WALK and client:WaterLevel() < 2 and not client:IsFlagSet(FL_ATCONTROLS) then
			local isYawing, gaincoeff = false, 0

			if bit.band(buttons, IN_LEFT) ~= 0  then isYawing = not isYawing end
			if bit.band(buttons, IN_RIGHT) ~= 0 then isYawing = not isYawing end
			if not (data.cvar.cl_yawspeed < 50.0 or isYawing == false) then
				data.iYawTickCount = data.iYawTickCount + 1
			end

			if IsTimerRunning(client) then -- ply.time
				data.iTimingTickCount = data.iTimingTickCount + 1
			end

			data.strafeTick = data.strafeTick + 1
			if data.strafeTick == 1000 then
				data.flRawGain = data.flRawGain * (998.0 / 999.0)
				data.strafeTick = data.strafeTick - 1
			end

			local velocity = client:GetAbsVelocity()
			local fore, side = angles:Forward(), angles:Right()
			local max_speed = client:GetMaxSpeed()
			fore.z, side.z = 0.0, 0.0
			fore:Normalize()
			side:Normalize()
			...
			if wishspeed > max_speed and max_speed ~= 0 then wishspeed = max_speed end
			if wishspeed ~= 0 then
				local SPEED_CAP = GetSpeedCap(client)
				local wishspd = (wishspeed > SPEED_CAP) and SPEED_CAP or wishspeed
				local currentgain = velocity:Dot(wishdir)

				if currentgain < SPEED_CAP then
					gaincoeff = (wishspd - math.abs(currentgain)) / wishspd
				end

				if gaincoeff > 0.5 then
					local mins, maxs = client:GetCollisionBounds()
					local pos = client:GetPos()
					local tr = utilTraceHull({
						start = pos, endpos = pos,
						mins = mins - VECTOR_1_1_0,
						maxs = maxs + VECTOR_1_1_0,
						mask = MASK_PLAYERSOLID_BRUSHONLY,
					})
					if tr.Hit then
						gaincoeff = math.abs(gaincoeff - 1)
					end
				end

				data.flRawGain = data.flRawGain + gaincoeff
			end
		end
