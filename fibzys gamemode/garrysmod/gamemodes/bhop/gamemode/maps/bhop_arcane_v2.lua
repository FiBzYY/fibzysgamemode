...
__HOOK[ "InitPostEntity" ] = function()
	...
	for _,ent in pairs( ents.FindByClass( "trigger_push" ) ) do
		if table.HasValue( pushes, ent:GetPos() ) then
			ent:Remove()
		end
	end
	
	for _,ent in pairs( ents.FindByClass( "func_button" ) ) do
		if table.HasValue( buttons, ent:GetPos() ) then
			ent:Remove()
		end
	end
	
	for _,ent in pairs( ents.FindByClass( "trigger_multiple" ) ) do
		if table.HasValue( deagles, ent:GetPos() ) or table.HasValue( weaponroom, ent:GetPos() ) then
			ent:Remove()
		end
	end
end