local st, lp, Iv, ey, ler, con, gt, near, col, cw, cd, rat, fr, lst = SysTime, LocalPlayer, IsValid, EyeAngles, Lerp, TIMER:ConvertTime(), TIMER.GetTimeDifference, { pos = Vector() }, { r = 255, g = 0, b = 0, a = 80 }, Color( 255, 255, 255 ), Color( 40, 40, 40 ), 100, 150
...
hook.Add("HUDPaint", "PaintReplayRoute", function()
	if IsValid( near.ent ) and near.ent.vis and not near.ent.nohud then
		local e, dh, h = near.ent, ScrH() / 2, 18
		local d1 = "Replay " .. e.drawvel .. " (Delta: " .. math.Round( lp():GetVelocity():Length2D() - e:GetPosVel() ) .. " u/s)"
		...
		local d2, d3 = "Replay Time: " .. con( e.time ) .. gt( e.time ), "Your times:"
	...
	if dist < (near.pos - lp():GetPos()):Length() then
		...
		if Iv( e ) and e != self and e.rect then
			...
			if add and dat.Vel > 80 then
				...
				dat.Text = "- " .. con( dat.Time ) .. " [" .. (dat.DiffTime > 0 and "+" or "-") .. con( math.abs( dat.DiffTime ) ) .. "] (Replay Velocity " .. (dat.DiffVel > 0 and "+" or "") .. math.Round( dat.DiffVel ) .. " u/s)"
